...
def calc_num_micro_batch(seq_len_effective, max_token_len):
    seq_len_sorted = sorted(seq_len_effective.tolist(), reverse=True)
    bins = []
    for l in seq_len_sorted:
        found = False
...
def rearrange_micro_batches(
    ...
    num_micro_batches = calc_num_micro_batch(seq_len_effective, max_token_len)
